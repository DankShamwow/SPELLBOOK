...
func _ready() -> void:
	status_id = 3
	status_name = "Plagued"
	status_description = "Plagued Tiles spread their disease to adjacent tiles at the start of the victim's turn, and are worth one less point."
...
func on_application(_status_amount: int, _status_duration: int = 0):
	...
	if inflicted_entity is Character:
		...
		for i in tiles_in_play.size():
			...
			if not current_tile.type == LetterTile.TileType.PLAGUED and not current_tile.is_debuff_immune:
...
func on_turn_start(which: GameEntity, _count: int):
	if which == inflicted_entity:
		...
		for i in affected_tile_list.size():
			var potential_tiles = []
			var spreader_tile = affected_tile_list[i]
			
			#region Affects Character
			if inflicted_entity is Character:
				var spreader_index = spreader_tile.grid_index
				...
				if not current_grid_tiles.has(spreader_tile.tile_index):
					continue
					
				for j in tiles_in_play.size():
					var potential_infection = tiles_in_play[j] as LetterTile
					
					if not potential_infection.type == LetterTile.TileType.PLAGUED \
					and not potential_infection.is_debuff_immune and ( \
					potential_infection.grid_index == spreader_index + 4 or \
					potential_infection.grid_index == spreader_index - 4 or \
					potential_infection.grid_index == spreader_index + 1 or \
					potential_infection.grid_index == spreader_index - 1 ):
						potential_tiles.append(potential_infection)
						
				if not potential_tiles.size() == 0 and spreader_tile.type == LetterTile.TileType.PLAGUED:
			...
			#region Affects Enemy
			if inflicted_entity is Enemy:
				var spreader_index = spreader_tile.tile_index
				
				for j in inflicted_entity.enemy_deck.size():
					var potential_infection = inflicted_entity.enemy_deck[j] as LetterTile
					
					if not potential_infection.type == LetterTile.TileType.PLAGUED \
					and not potential_infection.is_debuff_immune and ( \
					potential_infection.tile_index == spreader_index + 4 or \
					potential_infection.tile_index == spreader_index - 4 or \
					potential_infection.tile_index == spreader_index + 1 or \
					potential_infection.tile_index == spreader_index - 1 ):
						potential_tiles.append(potential_infection)
