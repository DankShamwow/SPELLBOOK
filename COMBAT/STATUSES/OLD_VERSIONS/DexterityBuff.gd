...
class_name DexterityBuff_OLD

func _ready():
	id = 13
	status_name = "Dexterity"
	if amount <= 1:
		status_description = str("Tiles are granted " + str(self.amount) + " additional point when the target is not an enemy.")
	else:
		status_description = str("Tiles are granted " + str(self.amount) + " additional points when the target is not an enemy.")
	tile_status = false
