...
func _ready() -> void:
	status_id = 2
	status_name = "Burning Tile"
	status_description = str("After " + str(self.duration) + " turns, Tiles inflicted by this debuff are Destroyed. Destroyed Tiles are not returned to your Tile Bag for the remainder of combat.")
...
func on_application(_status_amount: int, _status_duration: int = 0):
	...
	if inflicted_entity is Character:
		...
		for i in tiles_in_play.size():
			...
			if not current_tile.type == LetterTile.TileType.BURNING and not current_tile.is_debuff_immune:
