...
func _ready() -> void:
	...
	_set_economy_rng(random_seed)
	_set_shop_rng(random_seed)
...
func _set_economy_rng(new_seed):
	economy_rng.set_seed(new_seed)
	
func _set_shop_rng(new_seed):
	shop_rng.set_seed(new_seed)
