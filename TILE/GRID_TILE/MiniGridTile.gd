...
func _on_tile_button_mouse_entered():
	...
	if self.grid_ghost == true:
		...
		GameEventHandler.mini_tile_hovered.emit(self.ghost_pair, true)
		GameEventHandler.tile_mini_tooltip_requested.emit(self.ghost_pair)
	else:
		original_z = self.z_index
		determine_tile_scale()
		self.z_index = 128
		GameEventHandler.mini_tile_hovered.emit(self, true)
		GameEventHandler.tile_mini_tooltip_requested.emit(self)
